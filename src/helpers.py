...
def build_training(y_labels, y_output, learning_rate=0.5):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=y_labels, logits=y_output))
    ...
    correct_prediction = tf.equal(
        tf.argmax(y_output, 1), tf.argmax(y_labels, 1))
...
def train_test_model(mnist, x_input, y_labels, accuracy, train_step, steps=1000, batch=100):
    ...
    print("Test accuracy: %g" % accuracy.eval(
        feed_dict={x_input: mnist.test.images, y_labels: mnist.test.labels}))
    print("Validation accuracy: %g" % accuracy.eval(feed_dict={
          x_input: mnist.validation.images, y_labels: mnist.validation.labels}))
...
def train_test_model_dropout(mnist, x_input, y_labels, accuracy, train_step, should_drop, steps=1000, batch=100):
    ...
    for i in range(steps):
        input_batch, labels_batch = mnist.train.next_batch(batch)
        feed_dict = {x_input: input_batch,
                     y_labels: labels_batch, should_drop: True}
        feed_dict_test = {x_input: input_batch,
                          y_labels: labels_batch, should_drop: False}
